...
class ReportGen(object):
    ...
    def generate_html_content(self, template_name, datas, escape=False):
        """
        ...
        try:
            t_templates_str = {
                "report_template.html": self.read_template_file(self.report_template_path),
    ...
    def convert_list_to_html(self, list_items):
        """
    ...
    def write_html_report(self, report, html_report_path):
        """
    ...
    def read_template_file(self, report_template_path):
        """
        read of the template.
        """
        try:
            with open(self.report_template_path) as f:
                return f.read()
        except Exception as e:
            util.mod_log(f"[-] ERROR in load_template: {str(e)}", util.FAIL)
            return ""

    def search_keywords_in_source(self, keyword, txt_ouput: bool = False):
        """
        ...
        for regexp in keyword_search_dict[keyword]:
            ...
            o.strip()
            if not txt_ouput:
                output += self.format_grep_output_for_html(o, regexp)
            else:
                output += self.format_grep_output_for_txt(o, regexp)

        return output

    def format_grep_output_for_txt(self, grep_result, regexp):
        ...
        try:
            ...
            for grep in grep_result.split("\n"):
                ...
                filepath = (
                    "source" + filepath[len(self.source_code_path) :]
    ...
    def format_grep_output_for_html(self, grep_result, regexp):
        """
        ...
        try:
            ...
            for grep in grep_result.split("\n"):
                ...
                output += self.generate_html_content(
                    "grep_lines.html",
    ...
    def extract_build_info(self):
        """
        This method is used to get build information from android manifest_data.xml.
        """
        try:
            version = self.manifest_data.attrib.get(
                "platformBuildVersionCode",
                self.manifest_data.attrib.get("compileSdkVersion", "?"),
            )
            return version
    ...
    def get_permissions(self, manifest_data):
        """
        This method is used to extract permissions from the android manifest_data.xml.
        """
        try:
            permissions = []
            for permission_elem in self.manifest_data.findall(".//uses-permission"):
    ...
    def get_dangerous_permissions(self, manifest_data):
        """
        This method is used to extracts dangerous permissions from the android  manifest_data.xml.
        """
        permissions = []
        try:
            for permission_elem in self.manifest_data.findall(".//uses-permission"):
    ...
    def convert_html_to_pdf_report(self, html_file, pdf_name):
        """
    ...
    def get_cleaned_apk_name(self, apk_name):
        """
    ...
    def create_json_report(self, json_response):
        """
        This function generates the json report based on the json output
        """
        clean_apk_name = self.get_cleaned_apk_name(self.apk_name)
    ...
    def create_obj_for_report(
        ...
        obj = ReportGen(apk_name, manifest_data, resources_path, source_code_path, report_template_path)
        permissions = obj.get_permissions(manifest_data)
        dangerous_permission = obj.get_dangerous_permissions(manifest_data)

        html_dict = {}
        html_dict["build"] = obj.extract_build_info()
        html_dict["package_name"] = manifest_data.attrib["package"]
        html_dict["android_version"] = manifest_data.attrib["android:versionCode"]
        ...
        html_dict["intent_grep"] = obj.search_keywords_in_source("intent", txt_output)
        html_dict["internal_storage_grep"] = obj.search_keywords_in_source(
            "internal_storage", txt_output
        )
        html_dict["external_storage_grep"] = obj.search_keywords_in_source(
    ...
    def generate_txt_report(self, result_dict: dict):
        try:
            ...
            def _manif_analysis_parser(index: str) -> str:
                _res = f"\n{index.capitalize()}:\n"
                if len(result_dict["manifest_data_analysis"][index]["all"]) < 1:
                    _res += f"No {index} found.\n"
                for val in result_dict["manifest_data_analysis"][index]["all"]:
                    if val in result_dict["manifest_data_analysis"][index]["exported"]:
                        _res += f"- {val} [exported]\n"
                    else:
                        _res += f"- {val}\n"
                return _res
            ...
            # Saving the report
            cleaned_apk_name = obj.get_cleaned_apk_name(self.apk_name)

            if not os.path.isfile(self.output_path):
                txt_report_path = os.path.join(self.output_path, f"report_{cleaned_apk_name}.txt")
            else:
                txt_report_path = self.output_path
    ...
    def generate_html_pdf_report(self, report_type):
        ...
        try:
            ...
            cleaned_apk_name = obj.get_cleaned_apk_name(self.apk_name)
            if not os.path.isfile(self.output_path):
                html_report_path = os.path.join(self.output_path, f"report_{cleaned_apk_name}.html")
            else:
                html_report_path = self.output_path
            ...
            obj.write_html_report(report_content, html_report_path)
            if report_type == "html":
            ...
            if report_type == "pdf":
                ...
                obj.convert_html_to_pdf_report(html_report_path, pdf_path)
